    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        switch(URI_MATCHER.match(uri)) {
            ...
            case URI_SEARCH: {
                ...
                // TODO: Make MATCH support Chinese search.
                // We use LIKE instead of MATCH to support Chinese search as a workaround.
                // It may cost more time to complete a search when the table is large.
                String searchString = "%" + uri.getQueryParameter("pattern") + "%";
                //String searchString = uri.getQueryParameter("pattern") + "*";
                ...
                String smsQuery = String.format(
                        "SELECT %s FROM sms,words WHERE (words.index_text LIKE ? " +
            ...
            case URI_PENDING_MSG: {
                ...
                String subscription = uri.getQueryParameter("subscription");
                ...
                if (!TextUtils.isEmpty(subscription)) {
                    String subscriptionSelection = String.format(
                        "pending_msgs.msg_id IN (SELECT pdu._id FROM pdu " +
                        "WHERE pdu._id = pending_msgs.msg_id AND pdu.sub_id = %s)",
                        subscription
                    );
                    extraSelection += " AND " + subscriptionSelection;
                }
