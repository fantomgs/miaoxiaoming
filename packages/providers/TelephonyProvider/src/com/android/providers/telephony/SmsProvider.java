...
public class SmsProvider extends ContentProvider {
    ...
    private final static String[] ICC_COLUMNS = new String[] {
        ...
        "type",                         // according to getStatusOnIcc
    ...
    private ArrayList<String> convertIccToSms(SmsMessage message) {
        return convertIccToSms(message, SmsManager.getDefault().getPreferredSmsSubscription());
    }

    private ArrayList<String> convertIccToSms(SmsMessage message, int subscription) {
        ...
        result.add((type == Sms.MESSAGE_TYPE_INBOX) ? message.getDisplayOriginatingAddress()
        ...
        result.add(message.getStatusOnIcc());
        ...
        result.add(subscription);  // sub_id
        return result;
    }
    ...
    private Cursor getSingleMessageFromIcc(String messageIndexString) {
        return getSingleMessageFromIcc(messageIndexString,
                    SmsManager.getDefault().getPreferredSmsSubscription(), ICC_URI);
    }
    ...
    private Cursor getSingleMessageFromIcc(String messageIndexString, int subscription, Uri iccUri) {
        try {
            ...
            ArrayList<SmsMessage> messages = smsManager.getAllMessagesFromIcc(subscription);
            ...
            singleRow.add(convertIccToSms(message, subscription));
            return withIccNotificationUri(
                    new ArrayListCursor(ICC_COLUMNS, singleRow), iccUri);
    ...
    private Cursor getAllMessagesFromIcc() {
        return getAllMessagesFromIcc(SmsManager.getDefault().getPreferredSmsSubscription(), ICC_URI);
    }
    ...
    private Cursor getAllMessagesFromIcc(int subscription, Uri iccUri) {
        ...
        for (int count = messages.size(), i = 0; i < count; i++) {
            ...
            if (message != null) {
                rows.add(convertIccToSms(message, subscription));
            }
        }
        return withIccNotificationUri(new ArrayListCursor(ICC_COLUMNS, rows), iccUri);
    }
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (table.equals(TABLE_SMS)) {
            ...
            Integer subId = values.getAsInteger(Sms.SUB_ID);
            if (subId == null) {
                values.put(Sms.SUB_ID, SmsManager.getDefault().getPreferredSmsSubscription());
            }
    ...
    private int deleteMessageFromIcc(String messageIndexString) {
        return deleteMessageFromIcc(messageIndexString, SmsManager.getDefault().getPreferredSmsSubscription(), ICC_URI);
    }
