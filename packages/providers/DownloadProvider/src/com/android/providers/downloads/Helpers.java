...
public class Helpers {
    ...
    private static void checkCanHandleDownload(Context context, String mimeType, int destination,
        ...
        if (destination == Downloads.Impl.DESTINATION_EXTERNAL
            ...
            /* when start to download, it will check whether there is activity to deal with
               this file type, if no activity, it will download failed. So here we comment
               these codes, the result is that every file will be download.
             */
    ...
    private static File locateDestinationDirectory(Context context, String mimeType,
        ...
        return getExternalDestination(contentLength,mimeType);
    }

    private static File getExternalDestination(long contentLength,String mimeType) throws GenerateSaveFileError {
        if (!isExternalMediaMounted()) {
            throw new GenerateSaveFileError(Downloads.Impl.STATUS_DEVICE_NOT_FOUND_ERROR,
                    "external media not mounted");
        }
        ...
        File base = null;
        if (FeatureQuery.FEATURE_DOWNLOADPROVIDER_DESTINATION_FOLDER_BY_MIME) {
            String DownloadFolder = Constants.DEFAULT_DL_SUBDIR;
            ...
            base = new File(root.getPath() + DownloadFolder);
        } else {
