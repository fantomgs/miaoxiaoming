...
public class Workspace extends ViewGroup implements DropTarget, DragSource, DragScroller {
    ...
    void setCurrentScreen(int currentScreen) {
        ...
        setIndicatorsLevel(mCurrentScreen);
    ...
    private void snapToScreen(int whichScreen, int velocity, boolean settle) {
        ...
        setIndicatorsLevel(mNextScreen);
    ...
    void removeItems(final ArrayList<ApplicationInfo> apps) {
        ...
        final TelephonyManager tm = (TelephonyManager)mLauncher.getSystemService(Context.TELEPHONY_SERVICE);
                            ...
                            if (Intent.ACTION_MAIN.equals(intent.getAction()) && name != null) {
                                for (String packageName: packageNames) {
                                    if (packageName.equals(name.getPackageName())) {
                                        if (packageName.equals(STK_PACKAGE_NAME) && tm.isMultiSimEnabled()) {
                                            Log.d(TAG,"tm.hasIccCard(0)" + tm.hasIccCard(0) +
                                                "tm.hasIccCard(1)" + tm.hasIccCard(1));
                                            if (STK1_NAME.equals(name.getClassName()) && !tm.hasIccCard(0)) {
                                                Log.d(TAG,"removeItems stk1");
                                                LauncherModel.deleteItemFromDatabase(mLauncher, info);
                                                childrenToRemove.add(view);
                                            } else if (STK2_NAME.equals(name.getClassName()) && !tm.hasIccCard(1)) {
                                                Log.d(TAG,"removeItems stk2");
                                                LauncherModel.deleteItemFromDatabase(mLauncher, info);
                                                childrenToRemove.add(view);
                                            }
                                        } else {
                ...
                if (tag instanceof ShortcutInfo) {
                    ...
                    if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION &&
                        ...
                        for (int k=0; k<appCount; k++) {
                            ...
                            if (STK_PACKAGE_NAME.equals(name.getPackageName()) && tm.isMultiSimEnabled()) {
                                Log.d(TAG,"tm.hasIccCard(0)" + tm.hasIccCard(0)
                                         + "tm.hasIccCard(1)" + tm.hasIccCard(1));
                                if (STK1_NAME.equals(name.getClassName()) && !tm.hasIccCard(0)) {
                                    Log.d(TAG,"updateShortcuts removeItems stk1");
                                    LauncherModel.deleteItemFromDatabase(mLauncher, info);
                                    childrenToRemove.add(view);
                                } else if (STK2_NAME.equals(name.getClassName()) && !tm.hasIccCard(1)) {
                                    Log.d(TAG," updateShortcuts removeItems stk2");
                                    LauncherModel.deleteItemFromDatabase(mLauncher, info);
                                    childrenToRemove.add(view);
                                }
    ...
    void setIndicators(Drawable[] indicators) {
        mIndicators = indicators;
        setIndicatorsLevel(mCurrentScreen);
    }

    void setIndicatorsLevel(int screen) {
        for (Drawable draw : mIndicators) {
             draw.setLevel(screen);
        }
    }
