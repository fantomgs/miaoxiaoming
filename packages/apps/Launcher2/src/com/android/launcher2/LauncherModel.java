...
public class LauncherModel extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if("com.android.contacts.shortcut.update".equals(action)){
            long contactsId = intent.getLongExtra("column", -1);
            Bitmap icon = (Bitmap) intent.getParcelableExtra("icon");
            if(contactsId != -1){
                updateContactShortcutInfo(context,contactsId,icon);
            }
        }
    }

    private void updateContactShortcutInfo(Context context, long id,
            Bitmap icon) {
        final Intent intent = new Intent(
                ContactsContract.QuickContact.ACTION_QUICK_CONTACT);
        intent.setData(Uri.parse("content://com.android.contacts/contacts/lookup/%/" + id));
        ...
        try {
            cursor = context.getContentResolver().query(
                    ...
                    "intent like ?", new String[] { intent.toUri(0) }, null);
            while (cursor.moveToNext()) {
                ContentValues values = new ContentValues();
                values.put("title", getContactsName(context, id));
                if(icon != null){
                    values.put(LauncherSettings.BaseLauncherColumns.ICON_TYPE,
                            LauncherSettings.BaseLauncherColumns.ICON_TYPE_BITMAP);
                    ItemInfo.writeBitmap(values, Utilities.createIconBitmap(new FastBitmapDrawable(icon), context));
                }
                context.getContentResolver().update(
                        LauncherSettings.Favorites.CONTENT_URI,
                        values,
                        "_id=?",
                        new String[] { cursor.getLong(cursor
                                .getColumnIndex("_id"))
                                + "" });
            }
        } finally {
    ...
    private String getContactsName(Context context, long columnId) {
        Cursor cursor = null;
        try {
            cursor = context.getContentResolver().query(
                    ContactsContract.Contacts.CONTENT_URI, null,
                    ContactsContract.Contacts._ID + "=?",
                    new String[] { columnId + "" }, null);
