...
public class ContactEditorView extends BaseContactEditorView implements OnClickListener {
    ...
    private String getCardType(int subscription) {
        TelephonyManager tm = (TelephonyManager)mContext.getSystemService(Context.TELEPHONY_SERVICE);
        String cardType = null;
        if (TelephonyManager.isMultiSimEnabled() && tm != null) {
            cardType = tm.getCardTypeOnSubscription(subscription);
        } else if (!TelephonyManager.isMultiSimEnabled() && tm != null) {
            cardType = tm.getCardType();
        }
        return cardType;

    }

    //TODO: need to check csim when the framework support csim
    private boolean is3GCard(int subscription) {
        String type = getCardType(subscription);
        if (SimContactsConstants.USIM.equals(type))
            return true;
        return false;
    }
    ...
    public void setState(EntityDelta state, ContactsSource source, ViewIdGenerator vig) {
        ...
        CharSequence accountTypeLable = source.getDisplayLabel(mContext);
        if (TextUtils.isEmpty(accountTypeLable)) {
            accountTypeLable = mContext.getString(R.string.account_phone);
        }
        if (!TextUtils.isEmpty(accountName)) {
			if(SystemProperties.get("ro.product.customer_id").equals("JC_A107")||SystemProperties.get("ro.product.customer_id").equals("W706"))
			{
				mHeaderAccountName.setText(
                    mContext.getString(R.string.from_account_format, "UIM"));
			}
			else
			{
            	mHeaderAccountName.setText(
        ...
        mHeaderAccountType.setText(mContext.getString(R.string.account_type_format, accountTypeLable));
        ...
        if (mIsSourceReadOnly) {
            ...
            mReadOnly.setText(mContext.getString(R.string.contact_read_only, accountTypeLable));
        ...
        for (DataKind kind : source.getSortedDataKinds()) {
            ...
            if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType)) {
                ...
                if (SimContactsConstants.ACCOUNT_TYPE_SIM.equals(accountType) ) {
                    ...
                    if (!is3GCard(sub)) {
                        section.setTypeList(false);
                        section.setTypeOverallMax(1);
                    } else {
                        section.setTypeList(true);
                        section.setTypeOverallMax(2);
                    }
                }
                section.setState(kind, state, mIsSourceReadOnly, vig);
                parent.addView(section);
