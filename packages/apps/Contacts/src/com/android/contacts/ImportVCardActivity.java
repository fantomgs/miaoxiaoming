...
public class ImportVCardActivity extends Activity {
    ...
    private class VCardReadThread extends Thread
        ...
        private boolean readOneVCardFile(Uri uri, int vcardType,
            ...
            InputStream is;
            try {
                if (mCanceled) {   //need exit this work thread when user cancel the operation before start read stream.
                    return false;
                }

                is = mResolver.openInputStream(uri);

                //need exit this work thread when user cancel the operation in reading stream, Note, after reading stream, the mVCardParser instance will be non-NULL
                // then the mVCardParser.Cancel can Cancel the operation.
                synchronized(this) {  //here use synchronized to make sure user's cancel operation should not happened inside this block
                    if (mCanceled) {
                        return false;
                    }
        ...
        public void cancel() {
            synchronized(this) { //work together with another synchronized block.
                mCanceled = true;
