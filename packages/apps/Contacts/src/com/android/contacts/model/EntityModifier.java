...
public class EntityModifier {
    ...
    public static void trimEmpty(EntityDelta state, ContactsSource source) {
        ...
        for (DataKind kind : source.getSortedDataKinds()) {
            ...
            for (ValuesDelta entry : entries) {
                ...
                if ((entry.isDelete() || EntityModifier.isEmpty(entry, kind)) && !isGooglePhoto) {
    ...
    public static void parseExtras(EntityDelta state, DataKind kind, Bundle extras,
        ...
        if (canInsert && value != null) {
            if (value instanceof String && value.toString().indexOf("|") != -1) {
                for (Object c : value.toString().split("\\|"))
                    parseExtraIntoField(state, kind, extras, (CharSequence) c, typeExtra,
                            valueColumn);
            } else
                parseExtraIntoField(state, kind, extras, value, typeExtra, valueColumn);
        }
    }

    private static void parseExtraIntoField(EntityDelta state, DataKind kind, Bundle extras,
            CharSequence value, String typeExtra, String valueColumn) {
