...
public class FallbackSource extends ContactsSource {
    ...
    protected static final int FLAGS_GROUPS_ID = EditorInfo.TYPE_CLASS_NUMBER;
    ...
    protected void inflate(Context context, int inflateLevel) {
        ...
        inflateLocalGroups(context, inflateLevel);
    ...
    protected DataKind inflateLocalGroups(Context context, int inflateLevel) {
        DataKind kind = getKindForMimetype(LocalGroup.CONTENT_ITEM_TYPE);
        if (kind == null) {
            kind = addKind(new DataKind(LocalGroup.CONTENT_ITEM_TYPE, R.string.label_local_groups,
                    R.drawable.ic_list_allfriends_external, 110, true));
            kind.typeOverallMax = 1;
            kind.isList = true;
            kind.secondary = true;
            kind.actionHeader = new SimpleInflater(R.string.label_local_groups);
            kind.actionBody = new SimpleInflater(LocalGroup.GROUP);
        }

        if (inflateLevel >= ContactsSource.LEVEL_CONSTRAINTS) {
            kind.fieldList = Lists.newArrayList();
            kind.fieldList.add(new EditField(LocalGroup.GROUP, R.string.label_local_groups,
                    FLAGS_GROUPS_ID));
        }
