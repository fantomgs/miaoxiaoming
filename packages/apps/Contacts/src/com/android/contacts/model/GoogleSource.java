...
public class GoogleSource extends FallbackSource {
    ...
    public static final class LocalGroupNames {
        private static Map<String, Integer> groupNames = new HashMap<String, Integer>();
        ...
        public static void init(Context context) {
            ...
            String[] origNames = res.getStringArray(R.array.origin_group_names);
            String[] localeNames = res.getStringArray(R.array.locale_group_names);
            Integer localeId = null;
            for (int i = 0; i < origNames.length; i++) {
                localeId = new Integer(res.getIdentifier(localeNames[i], "string", context.getPackageName()));
                groupNames.put(origNames[i], localeId);
            }
        }

        public static CharSequence getLocalGroupName(CharSequence name) {
            Integer locale = null;
            synchronized (groupNames) {
                locale = groupNames.get(name);
                if (locale != null) {
                    try {
