...
public class PhoneDisambigDialog implements DialogInterface.OnClickListener,
    ...
    private boolean mEditBeforeCall;
    ...
    public PhoneDisambigDialog(Context context, Cursor phonesCursor, boolean sendSms,
            int stickyTab, boolean ipCall, boolean editBeforeCall) {
        mContext = context;
        mSendSms = sendSms;
        mIPCall = ipCall;
        mEditBeforeCall = editBeforeCall;
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (mPhoneItemList.size() > which && which >= 0) {
            ...
            if (mSendSms) {
                ...
                ContactsUtils.initiateCall(mContext, phone, mIPCall, mEditBeforeCall);
            }
        } else {
            dialog.dismiss();
        }
