...
public class NotificationMgr implements CallerInfoAsyncQuery.OnQueryCompleteListener{
    ...
    void notifyMissedCall(String name, String number, String label, long date, int subscription) {
        ...
        mNumberMissedCalls++;
        Log.d(LOG_TAG, "notifyMissedCall mNumberMissedCalls="+mNumberMissedCalls);
        ...
        Log.d(LOG_TAG, "MISSED_CALL_NOTIFICATION mNumberMissedCalls="+mNumberMissedCalls);

        if(SystemProperties.get("ro.product.customer_id").equals("W602"))
        {
            Intent missCallIntent = new Intent(Intent.ACTION_MISSCALL_MODE_CHANGED);
            //intent.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
            Log.d(LOG_TAG, "notifyMissedCall hasMissCall true");

            missCallIntent.putExtra("hasMissCall", true);
            mContext.sendBroadcast(missCallIntent);
        }
/*
        
        if(SystemProperties.get("ro.product.customer_id").equals("W602"))
        {
            Log.d(LOG_TAG, "MissedCalls set1ToMisscallBlink");//add xuhui
            byte[] LIGHT_ON = {'1'};              
            try{
                FileOutputStream fp = new FileOutputStream("/sys/class/leds/misscall/blink");
                fp.write(LIGHT_ON );
                fp.close();
            }catch(Exception e)
            {
    ...
    void cancelMissedCallNotification() {
        Log.d(LOG_TAG, "cancelMissedCallNotification ");
        /*
        // reset the number of missed calls to 0.
        if(SystemProperties.get("ro.product.customer_id").equals("W602"))
        {
            Log.d(LOG_TAG, "stat_sys_no_sim set0ToMisscallBlink");//add xuhui
            byte[] LIGHT_OFF = {'0'};              
            try{
                FileOutputStream fp = new FileOutputStream("/sys/class/leds/misscall/blink");
                fp.write(LIGHT_OFF );
        ...
        if(SystemProperties.get("ro.product.customer_id").equals("W602"))
        {
            Intent missCallIntent = new Intent(Intent.ACTION_MISSCALL_MODE_CHANGED);
            //intent.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
            missCallIntent.putExtra("hasMissCall", false);
            mContext.sendBroadcast(missCallIntent);
        }        
    ...
    /* package */ void updateCfi(boolean visible, int subscription) {
        ...
        if (visible) {
            ...
            if (showExpandedNotification) {
                ...
                intent.putExtra(SUBSCRIPTION_ID, subscription);
                // Add subscription into the data to make different hashcodes
