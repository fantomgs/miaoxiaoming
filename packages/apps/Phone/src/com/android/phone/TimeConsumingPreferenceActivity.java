...
public class TimeConsumingPreferenceActivity extends PreferenceActivity
    ...
    //flag indicate whether busy reading or saving dialog is showing
    private boolean mIsShowingBusyDialog = false;
    ...
    public void onFinished(Preference preference, boolean reading) {
        ...
        //here we don not use "mIsForeground" but "mIsShowingBusyDialog", because
        //if the dialog is shown when this activity is foreground, and then it
        //goes to background, after resume again, the dialog will not dismiss.
        if (mBusyList.isEmpty() && mIsShowingBusyDialog) {
            if (reading) {
                dismissDialogSafely(BUSY_READING_DIALOG);
            } else {
                dismissDialogSafely(BUSY_SAVING_DIALOG);
            }
            mIsShowingBusyDialog = false;
        }
    }
