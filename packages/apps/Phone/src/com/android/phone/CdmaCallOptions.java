...
public class CdmaCallOptions extends PreferenceActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (!getResources().getBoolean(R.bool.config_voice_privacy_disable)) {
            mButtonVoicePrivacy = (CdmaVoicePrivacyCheckBoxPreference) findPreference(BUTTON_VP_KEY);
            // for voice privacy button, set its subscription standalone
            if (mButtonVoicePrivacy != null) {
                mButtonVoicePrivacy.setSubscription(subscription);
            }
        }
    ...
    private void initCallWaitingPref(int subscription) {
        ...
        prefCWAct.getIntent().putExtra(CallFeaturesSetting.SUBSCRIPTION_ID, subscription)
                             ...
                             .setData(Uri.fromParts("tel", "*74"/*HX MODIFY mCallOptionSettings.getActivateNumber()*/, null));
        ...
        prefCWDeact.getIntent().putExtra(CallFeaturesSetting.SUBSCRIPTION_ID, subscription)
                               .putExtra(CDMA_SUPP_CALL, true)
                               .setData(Uri.fromParts("tel", "*740"/*HX MODIFY mCallOptionSettings.getDeactivateNumber()*/, null));
