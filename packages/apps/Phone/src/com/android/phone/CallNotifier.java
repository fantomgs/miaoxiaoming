...
public class CallNotifier extends Handler
    ...
    // the original 500ms is too short, that the first query after power on is too long > 500ms, which will cause the first incomming call always fallback to default ringtone. enlarge to 3s.
    private static final int RINGTONE_QUERY_WAIT_TIME = 3000;  // msec
	...
	//OLD teime
	private int mOld_bright_time;
	private int mNew_bright_time;
    ...
    public void handleMessage(Message msg) {
		...
		Log.w(LOG_TAG, "***zzg CallNotifier handleMessage msg.what="+msg.what+"***");
        ...
        switch (msg.what) {
            case PHONE_NEW_RINGING_CONNECTION:
				...
				/*
					...
					{
						mOld_bright_time = Settings.System.getInt(PhoneApp.getInstance().getPhone().getContext().getContentResolver(),
                            Settings.System.SCREEN_OFF_TIMEOUT,0);
						mNew_bright_time = 5000;
						Settings.System.putInt(PhoneApp.getInstance().getPhone().getContext().getContentResolver(),
                            Settings.System.SCREEN_OFF_TIMEOUT, mNew_bright_time);

						log("PHONE_NEW_RINGING_CONNECTION: " + mOld_bright_time);
					}
					*/
            ...
            case PHONE_INCOMING_RING: 
                ...
                if (msg.obj != null && ((AsyncResult) msg.obj).result != null) {
                    ...
                    //only ring after query completed.
                ...
                try 
				{
					if(SystemProperties.get("ro.product.customer_tp").equals("FT5X06"))
					{
						...
						|| SystemProperties.get("ro.product.customer_id").equals("S600"))
					{
						String SYSFS_PALM_UPDATE = "/sys/class/ms-touchscreen-it7260/device/enable_palm_sens";
						FileOutputStream out3 = new FileOutputStream(SYSFS_PALM_UPDATE);
						byte[] inputBuf3 = {1};
            ...
            case PHONE_DISCONNECT:
				...
				try 
				{
					...
					if(SystemProperties.get("ro.product.customer_id").equals("JC_A107")||SystemProperties.get("ro.product.customer_id").equals("W706"))
					{
						mOld_bright_time = Integer.parseInt(readValue(s_valueName));
					    if(mOld_bright_time == 0)
					    {
					    	mOld_bright_time = 10000;
					    }
						log("PHONE_DISCONNECT: " + mOld_bright_time);
						Settings.System.putInt(PhoneApp.getInstance().getPhone().getContext().getContentResolver(),
	                            Settings.System.SCREEN_OFF_TIMEOUT, mOld_bright_time);
					}
					*/
					//add by yangdecai 
    ...
    //make toast to notify user when call forwarding.
    private void notifySuppServiceNotification(){
        String callForwardTxt = suppServiceNotificationToString(mApplication, suppSvcNotification);
        if(!callForwardTxt.isEmpty()){
            Toast.makeText(mApplication, callForwardTxt, Toast.LENGTH_LONG).show();
        }
        clearSuppSvcNotification();
    }

    //convert SuppServiceNotification to a string.
    public static String suppServiceNotificationToString(Context context, SuppServiceNotification supp){

        String callForwardTxt = "";
        if (supp != null && context != null) {
            switch (supp.notificationType) {
                ...
                case 0:
                    switch (supp.code) {
                        ...
                        default:
                            Log.d(LOG_TAG,"Received unsupported MO SS Notification :" + supp.code);
                            break;
                    }
                    break;
                ...
                case 1:
                    switch (supp.code) {
                        ...
                        default :
                            Log.d(LOG_TAG,"Received unsupported MT SS Notification :" + supp.code);
                            break;
                    }
                    break;

                default:
                    Log.e(LOG_TAG, "Received invalid Notification Type :" + supp.notificationType);
                    break;
            }
        }
    ...
    private void onPhoneStateChanged(AsyncResult r) {
		...
		Log.w("CallNotifier", "***zzg onPhoneStateChanged state="+state+"***");
    ...
    public void onQueryComplete(int token, Object cookie, CallerInfo ci) {
        if (cookie instanceof Long) {
            ...
            if (DBG) log("CallerInfo query complete (for CallNotifier), "
            ...
            if (isQueryExecutionTimeOK) {
                ...
                if (ci.contactRingtoneUri != null) {
                    if (DBG) log("custom ringtone found, setting up ringer to " + ci.contactRingtoneUri);
    ...
    private void onDisconnect(AsyncResult r) {
        ...
        if (c != null) {
...

            {
                ...
                final boolean isCdmaSuppCall = mApplication.isCdmaSuppCall();

                // Don't put OTA or CDMA Emergency calls or CDMA supp call into call log
                if (!(isOtaNumber || isEmergencyNumber && shouldNotlogEmergencyNumber) && !isCdmaSuppCall) {
