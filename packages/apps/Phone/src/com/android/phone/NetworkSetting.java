...
public class NetworkSetting extends PreferenceActivity
    ...
    private static final int MAX_SCAN_TIME_OUT = 3*60*1000; // 3 minutes
    ...
    protected void onCreate(Bundle icicle) {
        ...
        Telephony.LocaleCarrierNames.init(getApplicationContext(), "NetworkSetting");
    ...
    private void displayNetworkSelectionFailed(Throwable ex) {
                  ...
                  == CommandException.Error.ILLEGAL_SIM_OR_ME)
        {
			...
			{
				status = getResources().getString(R.string.not_allowed_uim);
        	}
			else
			{
    ...
    private void networksListLoaded(List<NetworkInfo> result, int status) {
        ...
        Telephony.LocaleCarrierNames.reload();
        if (mHandler.hasMessages(EVENT_SCAN_TIME_OUT))
            mHandler.removeMessages(EVENT_SCAN_TIME_OUT);
        ...
        if (status != NetworkQueryService.QUERY_OK) {
            ...
            if (result != null){
                ...
                for (NetworkInfo ni : result) {
                    ...
                    carrier.setTitle(Telephony.LocaleCarrierNames.getLocaleCarrierName(getNetworkTitle(ni)));
