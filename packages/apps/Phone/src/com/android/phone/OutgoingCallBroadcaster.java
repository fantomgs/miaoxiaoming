 ...
 * broadcast.
 */
public class OutgoingCallBroadcaster extends MultiSimDialerActivity {
    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public void doReceive(Context context, Intent intent) {
            ...
            if (mIPCall) {
                ...
                ipPrefix = Settings.System.getString(getContentResolver(), Settings.System.IPCALL_PREFIX[mSubscription]);
    ...
    private void startSipCallOptionsHandler(Context context, Intent intent,
        ...
        newIntent.putExtra(CDMA_SUPP_CALL, mCdmaSupp);
    ...
    private BroadcastReceiver screenListener = new BroadcastReceiver(){

        public void onReceive(Context context, Intent intent) {
            if(Intent.ACTION_SCREEN_OFF.equals(intent.getAction())){
                Log.w(TAG, "receive screen off and close the dialog.");
                finish();
            }
        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        registerReceiver(screenListener, new IntentFilter(Intent.ACTION_SCREEN_OFF));
        mIntent = getIntent();
        ...
        boolean emergencyNumber = (number != null) && PhoneNumberUtils.isEmergencyNumber(number);
        if ((!emergencyNumber) && TelephonyManager.isMultiSimEnabled() && promptEnabled &&
            ...
            // if preferred callback enabled, we need this subscription info
            int subscription = mIntent.getIntExtra(SUBSCRIPTION, INVALID_SUB);
            if (subscription != INVALID_SUB) {
                intent_extra.putExtra(SUBSCRIPTION, subscription);
            }
            int requestCode = 1;
            //startActivityForResult(intent_extra, requestCode);
            this.initMultiSimDialer(intent_extra);
        } else if (isCdmaSuppCall()) {
            mSubscription = mIntent.getIntExtra(SUBSCRIPTION, PhoneApp.getVoiceSubscription());
            Log.d(TAG, "subscription in cdma supp call "+ mSubscription);
            processIntent();
        } else {
            // mSubscription = PhoneApp.getVoiceSubscription();
            // if this call is initiated from the bluetooth handset to redial last call log, we will try to get
            // the original subscription info to MO call
            if (activeSubCount() > 1){
                if (DBG) Log.v(TAG, "active sub count > 1 and use original SLOT");
                if (isCallbackPriorityEnabled())
                    mSubscription = mIntent.getIntExtra(SUBSCRIPTION, PhoneApp.getVoiceSubscription());
                else
                    mSubscription = PhoneApp.getVoiceSubscription();
            } else {
                Log.w(TAG, "active sub count isn't more than 1! use voice subscription to call out!");
                mSubscription = PhoneApp.getVoiceSubscription();
            }
    ...
    protected void onPause() {
        super.closeMultiSimDialer();
        finish();
    }

    protected void onBack(int resultCode, Intent data) {
    ...
    private void processIntent() {
        mIPCall = mIntent.getBooleanExtra(IPCALL_KEY, false);
        ...
        broadcastIntent.putExtra(CDMA_SUPP_CALL, mCdmaSupp);
    ...
    private boolean isCdmaSuppCall() {
        Bundle extras = mIntent.getExtras();

        if (extras != null) {
            mCdmaSupp = extras.getBoolean(CDMA_SUPP_CALL, false);
            Log.d(TAG, "isCdmaSuppCall " + mCdmaSupp);
        }
        return mCdmaSupp;
    }
