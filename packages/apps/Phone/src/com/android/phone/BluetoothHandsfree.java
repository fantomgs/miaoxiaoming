...
public class BluetoothHandsfree {
    ...
    /* package */ synchronized void audioOff() {
        ...
        if (mOutgoingSco == null && mConnectedSco == null){
            BluetoothDevice device = null;
            if (mHeadset != null) {
                device = mHeadset.getRemoteDevice();
            }
            broadcastAudioStateIntent(BluetoothHeadset.AUDIO_STATE_OUTGOINGSCO_DISCONNECTED, device);
        }
    ...
    private AtCommandResult redial() {
        ...
        Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                Uri.fromParts("tel", numsub.number, null));
        ...
        // if we get last call log successfully with the subscription info,
        // we need to use it to redial the contact
        if (numsub.sub != INVALID_SUB)
            intent.putExtra("Subscription",numsub.sub);
