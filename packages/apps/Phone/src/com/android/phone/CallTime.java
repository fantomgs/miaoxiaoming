...
public class CallTime extends Handler {
	...
	private	static boolean m_power = false;		// HX ADD
    ...
    private void updateElapsedTime(Call call) {
        if (mListener != null) {
		int mOldBrightness = 115;
		try{	
	        	   mOldBrightness =  Settings.System.getInt(PhoneApp.getInstance().getPhone().getContext().getContentResolver(), 
	                Settings.System.SCREEN_BRIGHTNESS);//android.os.Power.BRIGHTNESS_ON;//
		}
		catch (SettingNotFoundException snfe) 
		{
			log("updateElapsedTime SettingNotFoundException");
		}
            long duration = getCallDuration(call);
            mListener.onTickForCallTimeElapsed(duration / 1000);
			// HX ADD
			//mOldBrightness = android.os.Power.BRIGHTNESS_ON;
			...
			{				
				if(SystemProperties.get("ro.product.customer_tp").equals("FT5X06"))
				{
					...
					FileInputStream in = new FileInputStream(SYSFS_PALM_UPDATE);
							...
							if ( m_power == true )
							{
								...
								try {
						            IPowerManager power = IPowerManager.Stub.asInterface(
						                    ServiceManager.getService("power"));
						            if (power != null) {
						                power.userActivity(SystemClock.uptimeMillis(),false);
						            }
						        } catch (RemoteException doe) {
								...
								*/
								...
								inputBuf3 = new byte[2];
								
								inputBuf3[0] = 2;
								inputBuf3[1] = (byte)mOldBrightness;
								out3.write(inputBuf3); 
								out3.close();
