...
class InCallMenu {
    ...
    /* package */ void initMenu() {
		...
		//menu菜单中的结束键文字颜色	
		mEndCall.setTextColor(0xFFFF6072);
    ...
    /* package */ boolean updateItems(CallManager cm) {
        ...
        if (!mInCallScreen.isTouchUiEnabled() &&
            hasActiveCall && TelephonyCapabilities.supportsOtasp(cm.getFgPhone()) &&
        ...
        if (hasRingingCall) {
            ...
            if (hasActiveCall && !hasHoldingCall) {
                ...
                if (phoneType == Phone.PHONE_TYPE_CDMA) {
                    //CDMA waiting call, if it is touch ui,there is no menu.
                    if (mInCallScreen.isTouchUiEnabled()) {
                        return false;
                    }

                    //if it is not touch ui, should show answer and ignore menu.
