...
public class PhoneInterfaceManager extends ITelephony.Stub {
            ...
            switch (msg.what) {
                ...
                case CMD_END_CALL:
                    ...
                    if (phoneType == Phone.PHONE_TYPE_CDMA) {
                        ...
                        // check phone state to judge whether to end the call
    ...
    public boolean isOffhook() {
        ...
        if (TelephonyManager.isMultiSimEnabled()) {            
            for (int i = 0; i < TelephonyManager.getPhoneCount(); i++) {
                 isOffhook = isOffhook || isOffhookOnSubscription(i);
            }
        } else {
            isOffhook = isOffhookOnSubscription(getDefaultSubscription());
        }
        return isOffhook;
    }
    ...
    public boolean isRinging() {
        ...
        if (TelephonyManager.isMultiSimEnabled()) {
            for (int i = 0; i < TelephonyManager.getPhoneCount(); i++) {
                 isRinging = isRinging || isRingingOnSubscription(i);
            }
        } else {
            isRinging = isRingingOnSubscription(getDefaultSubscription());
        }
        return isRinging;
    }
    ...
    public boolean isIdle() {
        ...
        if (TelephonyManager.isMultiSimEnabled()) {
            for (int i = 0; i < TelephonyManager.getPhoneCount(); i++) {
                isIdle = isIdle && isIdleOnSubscription(i);
            }
        } else {
            isIdle = isIdleOnSubscription(getDefaultSubscription());
        }
        return isIdle;
    }
    ...
    public boolean supplyPuk(String puk, String newPin) {
         return supplyPukOnSubscription(puk, newPin, getDefaultSubscription());
    }

    public boolean supplyPukOnSubscription(String puk, String newPin,
            int subscription) {
        enforceModifyPermission();
        final CheckSimPuk checkSimPuk = new CheckSimPuk(getPhone(subscription)
                .getIccCard());
        checkSimPuk.start();
    ...
    public void toggleRadioOnOffOnSubscription(int subscription) {
        enforceModifyPermission();
        getPhone(subscription).setRadioPower(!isRadioOnOnSubscription(subscription));
    }
    ...
    /**
     * {@hide}
     * Returns the state of the request subscription
     */
    public int getSubscriptionState(int subscription) {
        return PhoneApp.getSubscriptionState(subscription);
    }
}
