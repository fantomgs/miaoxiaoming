...
public class PhoneApp extends Application implements AccelerometerListener.OrientationListener {
	...
	public static String sCleanNetworkLock = "com.android.phone.CleanNetworkLock";
    ...
    public void onCreate() {
        ...
        if (phone == null) {
			...
			intentFilter.addAction(sCleanNetworkLock);
            if (mTtyEnabled) {
			...
			if(SystemProperties.get("ro.product.customer_id").equals("S500"))
			{
				// set date
				Calendar c = Calendar.getInstance();
				Log.i(LOG_TAG,"year:"+c.get(Calendar.YEAR) + "month="+c.get(Calendar.MONTH) );
    ...
    boolean isCdmaSuppCall() {
        if (mInCallScreen == null) return false;
        Log.e("czhu", " PhoneAPP");
        return mInCallScreen.isCdmaSuppCall();
    }
    ...
    private void updatePokeLock() {
        ...
        switch (mScreenTimeoutDuration) {
            case SHORT:
                ...
                // has been released.
                pokeLockSetting |= LocalPowerManager.POKE_LOCK_IGNORE_CHEEK_EVENTS;
                break;
    ...
    private class PhoneAppBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
                ...
                if (phone.getPhoneType() == Phone.PHONE_TYPE_CDMA) {
                    ...
                    if (intent.getBooleanExtra("phoneinECMState", false)) {
                        ...
                        ecbmIntent = new Intent(EmergencyCallbackModeExitDialog.ACTION_SHOW_ECM_EXIT_DIALOG);
                        ecbmIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        ecbmIntent.putExtra("Subscription", subscription);
                        context.startActivity(ecbmIntent);
                    }
                } else {
                ...
                }else if( mCM.getState() == Phone.State.RINGING && ringerMode == AudioManager.RINGER_MODE_NORMAL){
                    notifier.startRinger();
                }
			...
			else if(action.equals(sCleanNetworkLock)){
				getPhone().getIccCard().disableNetworkLock();
				}
        }
    }
    ...
    private class MediaButtonBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ((event != null)
                ...
                boolean consumed = PhoneUtils.handleHeadsetHook(getPhone(mCM.getPhoneInCall().getSubscription()), event);
...
// ifdef CUST_EDITION
		else if(IccCard.INTENT_VALUE_LOCKED_NETWORK.equals(reason))
		{
...
// endif
    ...
    public static int getSubscriptionState(int subscription) {
        return PhoneFactory.getSubscriptionState(subscription);
	}

	public static boolean isSubscriptionActive(int subscription) {
        return PhoneFactory.isSubscriptionActive(subscription);
	}

	public static boolean isMultiSimsAvailable() {
        return PhoneFactory.isMultiSimsAvailable();
	}
