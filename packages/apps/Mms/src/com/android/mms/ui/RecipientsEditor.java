...
public class RecipientsEditor extends MultiAutoCompleteTextView {
    ...
    public RecipientsEditor(Context context, AttributeSet attrs) {
        ...
        // start show the drop down list by one character input
        setThreshold(1);
    ...
    private boolean hasInvalidCharacter(String number) {
        ...
        for (int i=0; i<count; i++) {
            if (charNumber[i] >= SBC_CHAR_START && charNumber[i] <= SBC_CHAR_END) {
                return true;
            }
        }
        return false;
    ...
    public int hasValidRecipient(boolean isMms) {
...
//        return false;
	...
	public void appendTextForUser(CharSequence text) {
		Editable editable = getText();
		if (!mTokenizer.isEndWithSeparator(editable)) {
			editable.append(mLastSeparator + " ");
		}
		
        editable.append(mTokenizer.terminateToken(text));
	}
    ...
    private class RecipientsEditorTokenizer
		...
		public boolean isEndWithSeparator(CharSequence text) {
			int i = text.length();
			char c = ' ';
			while (i > 0 && (c = text.charAt(i - 1)) == ' ') {
				i--;
			}
			if (i == 0 || c == ',' || c == ';')
				return true;
