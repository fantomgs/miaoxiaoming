...
public class TransactionService extends Service implements Observer {
    ...
    private static final int EVENT_MMS_CONNECTIVITY_TIMEOUT = 5;
    private static final int EVENT_QUIT = 100;

    private static final int MMS_CONNECTIVITY_DELAY = 75 * 1000;    // delay 75 seconds to check whether we have enabled mms connectivity
    ...
    private static final int TOAST_SWITCH_DATA_BACK_TIMEOUT = 3;
    private static final int TOAST_SWITCH_DATA_BACK_FAILED = 4;
    ...
    public Handler mToastHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            } else if (msg.what == TOAST_SWITCH_DATA_BACK_TIMEOUT) {
                str = getString(R.string.switch_data_back_timeout);
            } else if (msg.what == TOAST_SWITCH_DATA_BACK_FAILED) {
                str = getString(R.string.switch_data_back_failed);
            }
    ...
    public Handler mDataSwitchHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == EVENT_DATA_RESET) {
                ...
                if (MmsConfig.needDataSwitch()) {
                    Log.d(TAG, "switch back data failed. need switch it manually.");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_ONALARM.equals(intent.getAction()) || (intent.getExtras() == null)) {
            ...
            if (TelephonyManager.isMultiSimEnabled()) {
                cursor = PduPersister.getPduPersister(this).getPendingMessages(
                    System.currentTimeMillis(), MmsConfig.getDataSubscription());
            } else {
                cursor = PduPersister.getPduPersister(this).getPendingMessages(
    ...
    private void launchTransaction(int serviceId, TransactionBundle txnBundle, boolean noNetwork) {
        if (noNetwork) {
            ...
            int transactionType = txnBundle.getTransactionType();
            if (transactionType == Transaction.RETRIEVE_TRANSACTION) {
                DownloadManager.getInstance().markState(Uri.parse(txnBundle.getUri()),
    ...
    private final class ServiceHandler extends Handler {
                ...
                case EVENT_DATA_STATE_CHANGED:
                    ...
                    if ((info == null) || (info.getType() !=
                        ...
                        if (info != null && Phone.REASON_VOICE_CALL_ENDED.equals(info.getReason())
                                && !mPending.isEmpty()) {
                            // when network reconnect after VOICE_CALL_ENDED and there is pending transaction,
                            // we will try to enalbe mms connectivity.
                            try {
        ...
        private boolean isDuplicateRetrieve(Transaction t1, Transaction t2) {
            ...
            if ((t1 instanceof NotificationTransaction)
                ...
                if (((NotificationTransaction) t1).getUri().equals(
                        ((RetrieveTransaction) t2).getUri())) {
                    return true;
                }
            }
            ...
            if ((t1 instanceof RetrieveTransaction)
                    && (t2 instanceof RetrieveTransaction)) {
                if (((RetrieveTransaction) t1).getUri().equals(
                        ((RetrieveTransaction) t2).getUri())) {
                    return true;
                }
            }
            ...
            synchronized (mProcessing) {
                for (Transaction t : mPending) {
                    if (t.isEquivalent(transaction)) {
                        ...
                        return true;
                    } else if (isDuplicateRetrieve(t, transaction)) {
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || DEBUG) {
                ...
                for (Transaction t : mProcessing) {
                    if (t.isEquivalent(transaction)) {
                        ...
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || DEBUG) {
                            Log.v(TAG, "Duplicate Retrieve Transaction: " + transaction.getServiceId());
                        }
                        return true;
                    }
                }
