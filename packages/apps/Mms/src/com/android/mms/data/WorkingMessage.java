...
public class WorkingMessage {
    ...
    public interface MessageStatusListener {
		...
		void onToastTotalSize(int MmsSize); //add by pyuangui
    ...
    public int setAttachment(int type, Uri dataUri, boolean append) {
		...
		if (dataUri != null)
        {
        	Uri mImageUri = Uri.parse("content://media/external/images/media");
			Uri mVideoUri = Uri.parse("content://media/external/video/media");
			Uri mAudioUri = Uri.parse("content://media/external/audio/media");

			Uri mFinalImageUri = null;
			Uri mFinalVideoUri = null;
			Uri mFinalAudioUri = null;
			...
			if (scheme.equals("file"))
			{
				if (type == IMAGE)
				{                
					Cursor cursor = mContentResolver.query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, null, null,
					...
					while (!cursor.isAfterLast()) 
					{
						...
						if (dataUri.toString().substring(7).equals(data)) 
						{
							int ringtoneID = cursor.getInt(cursor.getColumnIndex(MediaStore.MediaColumns._ID));							
							
							mFinalImageUri = mImageUri.withAppendedPath(mImageUri, "" + ringtoneID);
							break;
						}
						cursor.moveToNext();
					}	
					...
					{
						dataUri = mFinalImageUri;
					}
				}
				else if (type == VIDEO)
				{
					Cursor cursor = mContentResolver.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, null, null,
					...
					while (!cursor.isAfterLast()) 
					{
						...
						if (dataUri.toString().substring(7).equals(data)) 
						{
							...
							mFinalVideoUri = mVideoUri.withAppendedPath(mVideoUri, "" + ringtoneID);
							break;
						}
						cursor.moveToNext();
					}					
					
					if (mFinalVideoUri != null)
					{
						dataUri = mFinalVideoUri;
					}					
				}
				else if (type == AUDIO)
				{
					Cursor cursor = mContentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, null, null,
					...
					while (!cursor.isAfterLast()) 
					{
						...
						if (dataUri.toString().substring(7).equals(data)) 
						{
							...
							mFinalAudioUri = mAudioUri.withAppendedPath(mAudioUri, "" + ringtoneID);
							break;
						}
						cursor.moveToNext();
					}	
					
					if (mFinalAudioUri != null)
					{
						dataUri = mFinalAudioUri;
					}					
				}
			}		 	
		}
    ...
    private void changeMedia(int type, Uri uri) throws MmsException {
        ...
        }
		{
	        if (MmsConfig.isRestrictedMode()) {
	            mmsTotalSize = media.getMediaSize();
	        } else {
	            ...
	            mmsTotalSize = media.getMediaResizable() ? 0 : media.getMediaSize();
	        }			
        }
		Log.d(TAG, "changeMedia mmsTotalSize="+mmsTotalSize);
		mmsTotalSize = mmsTotalSize + mSlideshow.getCurrentMessageSize();
		Log.d(TAG, "changeMedia mmsTotalSize="+mmsTotalSize);
		if(mmsTotalSize > MmsConfig.getMaxMessageSize())
		{
			Log.d(TAG, "too big");
		...
		Log.d(TAG, "changeMedia mmsTotalSize="+mmsTotalSize);
		mmsTotalSize = mmsTotalSize + mSlideshow.getCurrentMessageSize();
		if(mmsTotalSize > 307200 && type == IMAGE){
           mmsTotalSize = 307200;
		}
			
		mStatusListener.onToastTotalSize(mmsTotalSize);
		Log.d(TAG, "changeMedia mmsTotalSize="+mmsTotalSize);	
		//add end
	    // Add it to the slide.
	    slide.add(media);
     ...
     */
    private void prepareForSave(boolean notify, boolean removeEmptySubject) {
    ...
    public Uri saveAsMms(boolean notify) {
        ...
        prepareForSave(true /* notify */, false);
     ...
     */
    public void saveDraft(boolean waitForAttachment) {
        ...
        prepareForSave(false /* notify */, !waitForAttachment);
    ...
    */
    ...
    public void send(final String recipientsInUI) {
        ...
        prepareForSave(true /* notify */, true);
    ...
    private void clearConversation(final Conversation conv) {
        asyncDeleteDraftSmsMessage(conv);
        // Change the draft state before clear thread id.
