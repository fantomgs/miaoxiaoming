                ...
                if (candidates != null) {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        if (isNotRegularPhoneNumber) {
                            ...
                            // This is only for fetion, used to distinguish fetion style and direct style number.
                            // For one number, if candidate is fetion style but numberOrEmail is direct style, we 
                            // just use direct one, and if numberOrEmail is fetion style but candidate is direct style,
                            // we just use fetion style to send.
                            // FIXME: we will use numberOrEmail directly instead of candidate one if "12520" is not for 
                            // special usage.
                            if (((c.mNumber).startsWith(FETION_PREFIX)) && (!(numberOrEmail.startsWith(FETION_PREFIX)))) {
                                continue;
                            }
                            if (((numberOrEmail).startsWith(FETION_PREFIX)) && (!(c.mNumber.startsWith(FETION_PREFIX)))) {
                                continue;
                            }
                            // FIXME: Use more accurate compare method to check whether
                            // the numberOrEmail is same as or alias for c.mNumber.
                            if (PhoneNumberUtils.compare(numberOrEmail, c.mNumber) 
                                    || (c.mNumber != null 
                                        && c.mNumber.equals(PhoneNumberUtils.stripSeparators(numberOrEmail)))) {
                                return c;
                            }
                        }
                    }
