...
public class DownloadManager {
    ...
    private final BroadcastReceiver mRoamingStateListener =
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_SERVICE_STATE_CHANGED.equals(intent.getAction())) {
                ...
                // For dsds: we should check whether the subscription is defaul data sub.
                if (TelephonyManager.isMultiSimEnabled()) {
                    int subscription = intent.getIntExtra(SUBSCRIPTION, INVALID_SUB_ID);
                    if (subscription != TelephonyManager.getDefault().getPreferredDataSubscription()) {
                        if (LOCAL_LOGV) {
                            Log.d(TAG, "Not default sub. sub is " + subscription);
                        }
                        return;
                    }
                }
    ...
    static boolean isRoaming() {
        boolean roaming = TelephonyManager.getDefault()
            .isNetworkRoaming(TelephonyManager.getDefault().getPreferredDataSubscription());
        if (LOCAL_LOGV) {
