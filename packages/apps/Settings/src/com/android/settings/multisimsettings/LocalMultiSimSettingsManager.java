...
public class LocalMultiSimSettingsManager implements DialogInterface.OnClickListener {
    ...
    public int getPreferredSubscription(int listType) {
         ...
         switch (listType) {
             case MultiSimSettingsConstants.VOICE_SUBSCRIPTION_LIST:
                preferredSubscription = PhoneFactory.getVoiceSubscription();
                break;
             case MultiSimSettingsConstants.SMS_SUBSCRIPTION_LIST:
                preferredSubscription = PhoneFactory.getSMSSubscription();
                break;
             case MultiSimSettingsConstants.DATA_SUBSCRIPTION_LIST:
                preferredSubscription = PhoneFactory.getDataSubscription();
                break;
         }
    ...
    public void setPreferredSubscription(Message msg) {
        switch (msg.arg1) {
            case MultiSimSettingsConstants.VOICE_SUBSCRIPTION_LIST:
                PhoneFactory.setVoiceSubscription(msg.arg2);
                msg.sendToTarget();
                break;
            case MultiSimSettingsConstants.SMS_SUBSCRIPTION_LIST:
                PhoneFactory.setSMSSubscription(msg.arg2);
                msg.sendToTarget();
                break;
            case MultiSimSettingsConstants.DATA_SUBSCRIPTION_LIST:
                ...
                mProxyManager.setDataSubscription(msg.arg2, setDdsMsg);
                break;
            default:
    ...
    public boolean getSubscriptionStatus(int subscription) {
        ...
        return (ProxyManager.getInstance().getSubscriptionStatus(subscription) == ProxyManager.SUB_ACTIVATED);
    }
}
