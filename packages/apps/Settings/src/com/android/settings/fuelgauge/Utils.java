...
public class Utils {
    ...
    public static String formatElapsedTime(Context context, double millis) {
       ...
       /*
        *   adjust output time format to avoid string overlap on some UI.
        *   hour:minute will be shown when duration time is exceed one hour
        *   Day information will not be shown either.
        */
        /*
        ...
        */
        ...
        if (days > 0) {
            ...
            p = sb.lastIndexOf(" ");
            if (p > 0) {
                p = sb.substring(0, p).lastIndexOf(" ");
            }
        } else if (hours > 0) {
            sb.append(context.getString(R.string.battery_history_hours, hours, minutes, seconds));
            //ori format is XXh XXm XXs, return XXh XXm
            p = sb.lastIndexOf(" ");
            if (p > 0) {
                p = sb.substring(0, p).lastIndexOf(" ");
            }
            android.util.Log.e("ksui", "string: " + sb.toString() + " index:" + p);
        } else if (minutes > 0) {
