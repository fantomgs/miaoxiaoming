...
public class WifiSettings extends PreferenceActivity implements DialogInterface.OnClickListener {
    ...
    public static final boolean isSupportFMC = SystemProperties
            .getInt("ro.config.cwenable", 0)==1;

    public static String[] CT_AP = null;
    private boolean isCTAPExist = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // get the CT AP list from the resource
        // note: the string of CT AP should only be added in the English version
        CT_AP = getResources().getStringArray(R.array.ct_ap_list);
        isCTAPExist = (CT_AP != null) && (CT_AP.length > 0);
        addCTAPListIfNeed();
    }
    ...
    protected void onPause() {
        ...
        mScanner.pause();
        if (mDialog != null) {
            //the user input password dialog should be in the original state when the SIM welcome dialog popup and
            //dismissed ,to fix the CR 307417
            //mDialog.dismiss();
            //mDialog = null;
        }
        if (mResetNetworks) {
        ...
        if (info instanceof AdapterContextMenuInfo) {
            ...
            if (preference instanceof AccessPoint) {
                ...
                if (mSelected.networkId != -1) {
                    ...
                    if (isSupportFMC && isCTAPExist) {
                        for (String ctap : CT_AP) {
                            if (ctap.equals(mSelected.ssid)) {
    ...
    private void increatePriority() {
        if (!isSupportFMC || !isCTAPExist)
            ++mLastPriority;
        else
            mLastPriority += CT_AP.length + 1;
    }
    ...
    private void addCTAPListIfNeed() {
        if (!isSupportFMC || !isCTAPExist)
            return;
        ...
        for (String ctap : CT_AP) {
            ...
            for (WifiConfiguration cfg : configs) {
                if (AccessPoint.convertToQuotedString(ctap).equals(cfg.SSID)) {
                    bFind = true;
                    break;
                }
            }
            if (bFind)
                continue;
            WifiConfiguration config = new WifiConfiguration();
            config.SSID = AccessPoint.convertToQuotedString(ctap);
            config.allowedKeyManagement.set(KeyMgmt.NONE);
            int networkId = mWifiManager.addNetwork(config);
    ...
    private boolean isCTAP(String ssid, boolean isQuotedString) {
        String mySSID = ssid;
        if (isQuotedString) {
            mySSID = AccessPoint.removeDoubleQuotes(ssid);
        }
        if (mySSID == null)
            return false;
        for (String ctap : CT_AP) {
            if (ctap.equals(mySSID))
                return true;
        }
        return false;
    }

    // this function is used to set CT AP to highest priority
    private void changeCTAPPriorityIfNeed() {
        if (!isSupportFMC || !isCTAPExist)
            return;
        ...
        List<WifiConfiguration> configs = mWifiManager.getConfiguredNetworks();
        for (WifiConfiguration cfg : configs) {
            if (isCTAP(cfg.SSID, true) || cfg.status == WifiConfiguration.Status.CURRENT)
                continue;
            if (cfg.priority > maxPriority)
                maxPriority = cfg.priority;
        }

        // change CT AP's priority
        for (WifiConfiguration cfg : configs) {
            for (int j = 0; j < CT_AP.length; j++){
                String ctap = CT_AP[j];
                if (AccessPoint.convertToQuotedString(ctap).equals(cfg.SSID)
                        && cfg.status != WifiConfiguration.Status.CURRENT) {
                    cfg.priority = maxPriority+CT_AP.length-j;
    ...
    private void updateConnectionState(DetailedState state) {
        ...
        if (mResetNetworks && (state == DetailedState.CONNECTED ||
                state == DetailedState.DISCONNECTED || state == DetailedState.FAILED) && mNetworkId == mLastInfo.getNetworkId()) {
            updateAccessPoints();
            enableNetworks();
            mNetworkId = -1;
        }
