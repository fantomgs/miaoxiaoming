...
public class LatinIME extends InputMethodService
    ...
    public void onDisplayCompletions(CompletionInfo[] completions) {
        ...
        if (mCompletionOn) {
            ...
            CompletionInfo[] finalCompletions = new CompletionInfo[completions != null ? completions.length : 0];
            for (int i=0, j=0; i<(completions != null ? completions.length : 0); i++) {
                CompletionInfo ci = completions[i];
                if (ci != null) {
                    stringList.add(ci.getText());
                    finalCompletions[j] = ci;
                    j++;
                }
            }
            mCompletions = finalCompletions;
    ...
    // since our 27A screen landscape height is 480, larger than the value
    // R.dimen.max_height_for_fullscreen, this override method will block
    // Latin IME to enter the full screen mode and cause some display issues.
    // So just remove this method to use default way (enter the full screen mode
